...
`@script`
In this lesson, we cover a popular algorithm of the class of association rule mining methods, the apriori algorithm.
...
`@script`
The apriori algorithm goes through the entire itemset lattice (starting from the 1-itemset), explores and counts all of the neighbour nodes and then moves to the 2-itemsets, and so forth.
...
The algorithm starts by counting the number of occurences of A; in this case it appears 3 times. Similarly, we scan the dataset to obtain the support of items B, C and D. All itemsets are frequent as their support values satisfy the minimum support threshold. These itemsets are used to generate candidate 2-itemsets.
...
`@script`
For the 2-itemsets, neither (AC) nor (AD) satisfy the minimum support threshold. This means from the apriori principle that all their supersets are as well infrequent itemsets.
...
`@script`
The final set of frequent itemsets is shown in blue on the graph. 
...
`@script`
To generate association rules and steer the output of the set of extracted rules, the arguments parameter, control and appearance can be used. Parameters contain values such as support, confidence threshold and minimum length of rules. Control argument allows to influence the performance of the algorithm. Appearance allows to specify constraints on the right or left hand side of the extracted rules.
...
We use the "is.subset" function to figure out which items are subsets of which items and after obtain the list of redundant rules.
