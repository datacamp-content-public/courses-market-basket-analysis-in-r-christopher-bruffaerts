...
`@script`
In this lesson, we are going to cover one of most popular algorithm of the association rule mining, the so-called apriori algorithm. We are first going to understand the rationale behind this algorithm and afterwards see how to apply it with the arules package.
...
The apriori algorithm is one of the classic algorithms. Other methods for association rule mining exists such as the ECLAT or the FP-growth algorithms.
...
Remember that our goal is to extract meaningful rules from the transactional dataset.

We are going to use the itemset lattice we have seen previously to illustrate how apriori algorithm works. 
...
Count in an efficient manner - use the apriori algorithm
...
---
## Summarizing the apriori algorithm
...
Let us start with a standard call of the apriori function to retrieve the list of frequent itemsets. We set as parameters of the function: 1. the minimum support threshold and 2. target as frequent itemset.
...
`@script`
By inspecting the object, we obtain the frequent itemsets with their respective support and count (all counts are indeed greater or equal than 3). The R output corresponds exactly to the itemset lattice displayed on the right hand side of the slide.
...
The main parameters of the apriori function are the support, confidence and minimum length (of the rule). More information about available parameters can be found by ...
Let us try a first call of the apriori function where the inputs are the transactional dataset as well as the specified parameters.
...
**Excluding the set of redundant rules**
...
Therefore, we need to identify the interesting rules and removing redundant ones.
...
`@script`
In this example, rule #2 provides no extra knowledge in addition to rule #1, since...
When a rule (such as #2) is a super rule of another rule (#1) and the former has the same or a lower lift, the former rule is considered to be redundant.
