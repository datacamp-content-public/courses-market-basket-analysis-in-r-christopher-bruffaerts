...
`@script`
Once the itemsets of length 1 have been analyzed, we move on to itemsets of length 2. All candidates are generated based on the frequent 1-itemsets. The support values of the 6 candidates are shown on the graph. Both itemsets (AC) and (AD) do not satisfy the minimum support threshold meaning that from the apriori principle that all their supersets cannot be considered as frequent itemsets.
