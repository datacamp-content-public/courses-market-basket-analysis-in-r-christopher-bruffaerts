...
`@script`
To extract induced rules from the transactional dataset, the association rule mining can be decomposed into two subtasks. First, the set of frequent items has to be generated from the database of transactions. An itemset is considered as frequent if its support satisfies a minimum threshold value. 
...
The apriori algorithm is a level-wise algorithm, meaning that we first start with the itemsets of size 1. 
...
`@script`
Once the itemsets of length 1 have been analyzed, we move on to itemsets of length 2. All candidates are generated based on the frequent 1-itemsets and their support are shown on the graph. Both itemsets (AC) and (AD) do not satisfy the minimum support threshold meaning that from the apriori principle that all their supersets cannot be considered as frequent itemsets.
...
`@script`
Both infrequent itemsets (AC) and (AD) are colored in orange on the graph. Their respective supersets are as well colored in orange as these super-sets are themselves infrequent itemsets (based on the apriori principle). This allows to substantially reduce the exploration space.
...
`@script`
We start with a standard call of the apriori function to retrieve the list of frequent itemsets. We set as argument the transactional dataset as well as the following parameters: 1. the minimum support threshold denoted as "supp" and 2. the "target" as frequent itemset.
