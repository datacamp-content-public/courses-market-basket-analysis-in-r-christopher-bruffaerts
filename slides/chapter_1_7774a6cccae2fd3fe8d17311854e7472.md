...
Given the large number of rules extracted from the apriori function, it is necessary to get an overview of redundant rules and identify the interesting rules. What do we mean by "redundant" rule?
...
In this example, rule #2 and rule #3 provide no extra knowledge in addition to rule #1 since if "B implies A", it is natural that "BD" and "BCD" also imply A. Therefore the rule "B implies A" is the non-redundant rule, considered as the interesting one.
