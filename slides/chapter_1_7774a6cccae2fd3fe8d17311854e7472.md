...
`@script`
The main purpose of the association rule mining is to extract some induced rules from the itemsets over a transactional database and discover interesting relationships between items.

To reach this goal, the process can be decomposed into two subtasks. First, the set of frequent items has to be generated from the database of transactions. An itemset is considered as frequent if its support satisfies a minimum threshold value. 
...
`@part2`
**Advantages of the apriori**

- Simple and easy to understand algorithm
- Can be used on large transactional datasets

**Disadvantages of the apriori**

- Computational cost can be high, can be slow in some cases
- Expensive method to calculate support because the calculation has to go through the entire database
...
`@script`
If we repeat the same exercise as before but this time using as minimum support threshold the value of 2, we obtain such itemset lattice. The number of frequent itemsets has naturally increased. Hence, the lower is the minimum support threshold, the more scans the algorithm has to do and therefore the longer the algorithm takes.
Let us note that the main advantages of the apriori are the fact that it is an easy to understand algorithm which can be used on large transactional datasets. On the other hand, the apriori algorithm can be computationally heavy and slow.
...
`@script`
By inspecting the object just created, we obtain a dataframe containing the frequent itemsets with their respective support and count (all counts are indeed greater or equal than 3). The R output corresponds exactly to the itemset lattice displayed on the right hand side of the slide which we built earlier.
...
`@script`
After retrieving the set of frequent itemsets, we use the apriori function to generate association rules. To generate these rules, we specify the main parameters of the apriori function: both the minimum support threshold and the minimum confidence threshold (eventually, we can use the default parameters) as well the minimum length of the rule (a value of 2 ensures that there is at least one item for the precedent and the antecedent of the rule). More information about available parameters can be found by typing ?APparameter.

Let us use the apriori function where the inputs of the function are:
1) the transactional dataset
2) the specified parameters, passed as a list. Finally, we use the inspect function to examine the set of generated rules.
...
`@script`
The "inspect" function allows to fetch the extracted rules. For each rule, the support, confidence and lift measures are provided.
...
The rule "D implies B" is the rule with both the highest support and confidence.
...
For instance, in this case, we are interested in all rules where the right-hand side (the consequent of the rule) is B and for which the lift is greater than 1.
